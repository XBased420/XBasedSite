...
const valid = { name: 'Test Visitor', email: 'visitor@example.com', phone: '(214) 555-0100', business: 'Test Business', type: 'Barber', hasSite: 'No', needs: 'A booking request website.', budget: 'Not sure yet — tell me what it should cost', timeline: 'Few weeks', socials: '', source: '', website: '', elapsedSeconds: 30 };
function backend(options = {}) {
  const rows = [], emails = [], events = [], cache = new Map();
  const props = { SHEET_ID: 'test-sheet' };
...
for (const [name, data, options] of [
  ['too fast', { ...valid, elapsedSeconds: 2.99 }, {}],
  ['missing timing', { ...valid, elapsedSeconds: undefined }, {}],
  ['string timing', { ...valid, elapsedSeconds: '30' }, {}],
  ['negative timing', { ...valid, elapsedSeconds: -1 }, {}],
...
]) test(`${name} cannot write a lead or send email`, () => { const b = backend(options); assert.equal(b.submit(data).ok, true); assert.equal(b.rows.length, 0); assert.equal(b.emails.length, 0); });
test('repeat request is deduplicated for ten minutes', () => { const b = backend(); b.submit(valid); assert.equal(b.submit(valid).duplicate, true); assert.equal(b.rows.length, 1); assert.equal(b.emails.length, 2); });
test('same sender is throttled when changing a request within one minute', () => { const b = backend(); b.submit(valid); assert.equal(b.submit({ ...valid, needs: 'A different request.' }).throttled, true); assert.equal(b.rows.length, 1); assert.equal(b.emails.length, 2); });
